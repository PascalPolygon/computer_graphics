...
class Robot:
    ...
    def move(self, deg, deg2, deg3):
        ...
        for i in np.arange(start, deg, step):
            # rate(100)
            self.l1.rotate(angle=rads(step),axis=vector(0,0,1), origin=vector(axis_x, axis_y,0))
            self.theta1 = deg
            angle1 = rads(i)
            T0_1 = get_transformation(angle1, 0, self.p2.pos.y)
            # angle2 = rads(0)
            angle2 = rads(self.theta2)
            T1_2 = get_transformation(angle2, 0, self.l1.height+(2*self.p3.radius))
            # angle3 = rads(0)
            angle3 = rads(self.theta3)
            T2_3 = get_transformation(angle3, 0, self.l2.height+(2*self.p4.radius)) #Really doesn't make a difference p3 or p4 or p5
            # angle4 = rads(0)
            T3_4 = get_transformation(rads(0), 0, self.l3.height+(2*self.p5.radius))
            ...
            Tp3_l2 = get_transformation(rads(i), 0,(self.l2.height/2)+(self.p4.radius))
            ...
            Tp4_l3 = get_transformation(0, 0,(self.l3.height/2)+(self.p5.radius))
        ...
        for i in np.arange(start, deg, step):
            # rate(100)
            self.l2.rotate(angle=rads(step),axis=vector(0,0,1), origin=vector(axis_x, axis_y,0))
            self.theta2 = deg
            # angle1 = rads(0)
            T0_1 = get_transformation(angle1, 0, self.p2.pos.y)
            angle2 = rads(i)
        ...
        for i in np.arange(start, deg, step):
            # rate(100)
            self.l3.rotate(angle=rads(step),axis=vector(0,0,1), origin=vector(axis_x, axis_y,0))
            # print(i)
            self.theta3 = deg
            # angle1 = rads(0)
            T0_1 = get_transformation(angle1, 0, self.p2.pos.y)
